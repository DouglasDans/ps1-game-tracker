...
**Redesign TV do Detail (2026-07-12):** tela do jogo refeita em formato "painel de console" (referências: PS5 game hub, Steam Big Picture — ver guidelines 10-foot: corpo ≥22-28px em 1080p, safe area 5%). Root font-size 18→22px e `--side-pad` 64→96px (afeta todas as telas). Layout: coluna lateral fixa (cover 280px + insights: média/sessão, mais longa, melhor dia, primeira vez, dias jogados) + coluna principal rolável (título, tempo total como hero stat, sinopse com clamp de 6 linhas, últimas 8 sessões com barra proporcional à duração + contador das restantes, placeholder RetroAchievements). Backdrop com gradiente da cor dominante da cover, extraída via canvas em `extractDominantColor` (`utils.js`) — images.igdb.com envia `Access-Control-Allow-Origin: *`, então canvas não fica tainted; cache em Map por URL; fallback sem cover usa `cardGradient` atenuado. Logos de plataforma: nome em texto só aparece como fallback quando não há logo asset (sem duplicação logo+nome). Nota de teste: eventos de tecla do Chrome DevTools às vezes não acionam scroll — o caminho real é o Gamepad API despachando `KeyboardEvent` sintético, que funciona; testar scroll via `document.dispatchEvent`.

**Design system unificado — cards + rail (2026-07-12):** duas primitivas de layout padronizam todas as telas: `.stat-panel` (container de card para qualquer gráfico/stat/informação — fundo raised, header interno) e `.side-rail`/`.rail-item` (navegação vertical à esquerda, com estados `.active` = aplicado e `.focused` = foco do d-pad). Stats: todas as seções em panels. Library: filtros de plataforma + ordenação viraram rail vertical (← do grid entra no rail, ↑↓ navega, ✕ aplica, → volta; L1/R1 seguem ciclando filtro direto). Detail: rail de seções abaixo da cover (**Visão geral / Sessões / Conquistas**, padrão PS4) — header do jogo (título/meta/hero stats) persistente acima do conteúdo da aba; Visão geral = insights em cards + sinopse; Sessões = lista completa (o limite de 8 caiu — aba própria com scroll via → e ↑↓); Conquistas = placeholder da Fase 6 no lugar definitivo.

**Stats como página com abas verticais + L1/R1 (2026-07-12):** Stats saiu da home e virou rota própria (`screens/stats.js`) — o item do header deixou de ser scroll disfarçado. Navegação de topo estilo console: **L1/R1 alterna Games ↔ Stats** globalmente via `initPageSwitcher` em `app.js`, mas só quando a tela atual está em `TOP_PAGES` — Library (sub-tela de Games) mantém L1/R1 para filtro de plataforma sem conflito. Stats organizada em **abas verticais** (rail esquerda, padrão PS4): Visão geral (cards + mais jogados), Atividade (dia da semana + período do dia), Biblioteca (plataforma/gênero/desenvolvedora/década/modo em 2 colunas) — cada aba cabe numa tela sem scroll; ↑/↓ troca de aba. Plano futuro registrado: detail em abas verticais (Visão geral / Sessões / Conquistas) junto com a Fase 6.

**Backdrop dinâmico na home (2026-07-12):** o hero da home ganha gradiente com a cor dominante da cover do jogo selecionado (mesma `extractDominantColor` do detail, cache compartilhado — navegar o carrossel troca a cor instantaneamente após a primeira extração). Guarda por token contra respostas assíncronas fora de ordem ao navegar rápido. Tile Library faz fade-out do backdrop; jogo sem cover usa `cardGradient` atenuado.
...
**Redesign TV — import Turno 4/5 do Claude Design, backend (2026-07-27):** importado documento de exploração do claude.ai/design ("Interface de TV para PS1 Pro") com o próximo redesign de Home/Detail/Library/Stats — implementação em fases na branch `design/home-tv-turno4`. Fase 1 (backend): `enricher.py` passa a extrair também `involved_companies.publisher` (mesmo padrão do `developer`, tolerando lista vazia — indie/homebrew não tem publisher creditado; uma company pode ser developer E publisher ao mesmo tempo em título autopublicado). `publisher` novo campo em `games`. `playtime_summary` ganha `days_played` (`COUNT(DISTINCT DATE(started_at))`) e `summary` — o carrossel da Home precisa dessas infos por jogo sem round-trip extra por seleção (TV precisa responder instantâneo a ← →). `get_stats_summary` ganha `total_sessions` e `total_days_played` (dias distintos com qualquer sessão, **não** é a soma de `days_played` por jogo — um dia com sessão em 2 jogos diferentes conta 1 vez no total global). `compute_activity_patterns` (`daemon/activity.py`) ganha `by_day`: lista dos últimos 91 dias (janela fixa terminando hoje, fuso America/Sao_Paulo) com total de segundos por dia — alimenta o heatmap de atividade tipo GitHub contribution graph que entra no Stats global e no Detail por jogo. Frontend (concluído): chrome global com nav em pílula + ícones (Material Symbols Rounded vendorizado, `web/assets/fonts/`, sem CDN), Home (hero 2 colunas + grid 2×3 de stats), Detail (aba Estatísticas por jogo substitui Sessões — dashboard com mês/heatmap/ritmo/dia da semana/hora, tudo calculado no client a partir de `d.sessions`, sem endpoint novo), Library e Stats globais redesenhados.

**Fidelidade visual ao mock (2026-07-27):** primeira passada das fases de frontend reaproveitou classes/tokens já existentes no app (`.insight-card`, `.stat-panel`, cores roxas, `system-ui`) em vez dos valores literais do `.dc.html` — usuário corrigiu: o motivo de mandar o HTML do Claude Design é fidelidade exata, não uma reinterpretação. Correção: `Archivo` (400-800) e `Roboto Mono` (400-500) vendorizados localmente (`web/assets/fonts/archivo-latin.woff2`, `roboto-mono-latin.woff2` — cada arquivo é uma fonte variável do Google que cobre todos os pesos precisados, então só 2 arquivos ~35KB cada; subset "latin" já cobre acentuação PT-BR). `--bg`/`--bg-raised`/`--bg-card` trocados pros hex literais do mock (`#0b0d12`/`#0e1017`/`#171a22`); `--side-pad` virou 76px fixo (não mais `clamp()`) e as dimensões de card (`--card-w` etc.) passaram a bater com a variante `cardsE` do mock — o alvo é sempre TV 1920×1080 fixa, então não há motivo pra manter escala fluida. Anel de seleção (`.game-card.selected`, `.lib-card.selected`) virou branco (`--select`), não mais azul — o mock usa branco para indicar seleção/foco em todo lugar. Cada tela ganhou classes próprias pros componentes novos (`.hero-stat-tile*` na Home, `.pg-panel*` no dashboard do Detail e reaproveitado no Stats globais) em vez de continuar reaproveitando `.insight-card`/`.stat-panel`/`.section-header` — evita que ajustar uma tela vaze pra outra.

**Canvas de design ampliado para 2260×1271 (2026-08-01):** reverte a decisão de 2026-07-27 ("o alvo é sempre TV 1920×1080 fixa") — o canvas fixo escalado por `fitViewport()` (`web/app.js`) e `#viewport` (`web/style.css`) passou de 1920×1080 para 2260×1271 (mesma proporção 16:9). Motivo: no TV 1080p real a UI estava grande demais; o canvas maior faz tudo desenhar ~15% menor via downscale do `transform: scale()`, sem precisar tocar em nenhum valor de px do resto do stylesheet — só as duas constantes do canvas. Nenhum outro px hardcoded referenciava 1920/1080 além desses dois lugares. Junto: `.hero-platform-logo` (meta da Home) subiu de 40px para 30px de height e ganhou `max-width: none` — as logos de plataforma são wordmarks bem largos (proporção 3.5:1 a 7.2:1, ex: PS1 5.14:1), então o `max-width: 90px` herdado de `.platform-logo` sempre foi o gargalo real do `object-fit: contain`, não a altura; subir só a altura não tinha efeito visível algum até remover esse teto. Pra abrir espaço pro wordmark maior sem quebrar a linha de metadados, `genre` e `developer` no hero da Home (`web/screens/home.js`) passaram a mostrar só o primeiro valor (antes do `,`) em vez da lista IGDB completa — ex: "Platform, Sport, Adventure" → "Platform".
